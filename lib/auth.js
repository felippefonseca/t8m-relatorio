...
export function verifyCredentials(body = {}) {
  const expectedUsers = credentialList(process.env.CLIENT_USERNAME || "t8m").map(
    normalizeUsername
  );
  const expectedPasswords = credentialList(process.env.CLIENT_PASSWORD || "troque-esta-senha").map(
    normalizePassword
  );
  const inputUser = normalizeUsername(body.username);
  const inputPassword = normalizePassword(body.password);

  return (
    secureIncludes(inputUser, expectedUsers) &&
    secureIncludes(inputPassword, expectedPasswords)
  );
}
...
function secureIncludes(input, candidates) {
  return candidates.some((candidate) => secureEqual(input, candidate));
}
