...
export async function getDashboardData(options = {}) {
  ...
  try {
    ...
    if (cached) return mergeFreshFinance(cached, liveOptions);
...
export async function getMetaFinanceData(options = {}) {
  const accessToken = options.accessToken || process.env.META_ACCESS_TOKEN;
  const adAccountId = options.adAccountId || process.env.META_AD_ACCOUNT_ID;
  const hasMetaConfig = Boolean(accessToken && adAccountId);
  ...
  if (!hasMetaConfig) {
    ...
    return getDemoFinanceData(options);
  }

  try {
    const accountId = normalizeAdAccountId(adAccountId);
    const account = await fetchAccountSnapshot(accountId, { ...options, accessToken, adAccountId });
    return {
      source: {
        mode: "live",
        label: "Meta Ads | T8M",
        detail: "Financeiro atualizado diretamente pela API da Meta."
      },
    ...
    const demo = getDemoFinanceData(options);
    demo.source = {
      mode: "demo",
      label: "Demonstracao",
      detail: "A conexao com a Meta falhou; exibindo financeiro ficticio."
    };
    demo.error = error.message;
    return demo;
  }
...
async function getMetaDashboardData(options) {
  ...
  const account = await fetchAccountSnapshot(accountId, options);
  const accountPayload = mapAccount(account);
...
async function fetchAccountSnapshot(accountId, options) {
  const baseFields = [
    "id",
    "name",
    "account_id",
  ...
  const prepayFields = ["prepay_account_balance", "total_prepay_balance"];

  try {
    return await graphFetch(
      `/${accountId}`,
      { fields: [...baseFields, ...prepayFields].join(",") },
      options
    );
  } catch (error) {
    const fallback = await graphFetch(`/${accountId}`, { fields: baseFields.join(",") }, options);
    fallback.finance_warning =
      "A Meta nao liberou os campos de saldo pre-pago para esta credencial.";
    return fallback;
  }
}
...
function mapAccount(account = {}) {
  ...
  const apiBalance = metaMinorToNumber(account.balance, currency);
  const prepayAccountBalance = currencyAmountToNumber(account.prepay_account_balance, currency);
  const totalPrepayBalance = currencyAmountToNumber(account.total_prepay_balance, currency);
  const finance = buildFinanceSnapshot({
    amountSpent,
    apiBalance,
    currency,
    financeWarning: account.finance_warning || "",
  ...
  return {
    ...
    balance: finance.apiBalance,
    balanceLabel: finance.primaryLabel,
    balanceLow: finance.isLow,
    balanceAlertThreshold: finance.alertThreshold,
    amountSpent: finance.amountSpent,
    spendCap: finance.spendCap,
    remainingCap: finance.remainingCap,
    availableBalance: finance.primary,
    finance,
    isPrepay: Boolean(account.is_prepay_account),
    spendCapConfigured: finance.spendCapConfigured
  };
}
...
function buildFinanceSnapshot({
  ...
  const spendCapConfigured = Boolean(spendCap && spendCap > 0);
  const remainingCap =
    spendCapConfigured && amountSpent !== null ? Math.max(spendCap - amountSpent, 0) : null;
  let primaryValue = null;
  let primaryLabel = "Financeiro informado pela Meta";
  let primarySource = "unavailable";
  let confidence = "unavailable";
  let cardTitle = "Financeiro Meta";
  let isActionableBalance = false;

  if (totalPrepayBalance !== null) {
    primaryValue = totalPrepayBalance;
    primaryLabel = "Saldo pré-pago total informado pela Meta";
    primarySource = "total_prepay_balance";
    confidence = "direct";
    cardTitle = "Saldo atual Meta";
    isActionableBalance = true;
  } else if (prepayAccountBalance !== null) {
    primaryValue = prepayAccountBalance;
    primaryLabel = "Saldo pré-pago informado pela Meta";
    primarySource = "prepay_account_balance";
    confidence = "direct";
    cardTitle = "Saldo atual Meta";
    isActionableBalance = true;
  } else if (spendCapConfigured && amountSpent !== null) {
    primaryValue = remainingCap;
    primaryLabel = "Limite restante de gastos";
    primarySource = "spend_cap";
    confidence = "estimated";
    cardTitle = "Limite restante Meta";
    isActionableBalance = true;
  } else if (apiBalance !== null) {
    primaryValue = apiBalance;
    primaryLabel = "Valor de cobrança informado pela Meta";
    primarySource = "billing_balance";
    confidence = "billing";
    cardTitle = "Cobrança Meta";
  }

  return {
    primary: moneyPayload(primaryValue, currency),
    primaryLabel,
    primarySource,
    confidence,
    cardTitle,
    isLow: isActionableBalance && primaryValue !== null && primaryValue <= financeAlertThreshold,
    isActionableBalance,
    updatedAt: new Date().toISOString(),
    alertThreshold: moneyPayload(financeAlertThreshold, currency),
    amountSpent: moneyPayload(amountSpent, currency),
    apiBalance: moneyPayload(apiBalance, currency),
    prepayAccountBalance: moneyPayload(prepayAccountBalance, currency),
    spendCap: moneyPayload(spendCap, currency),
    remainingCap: moneyPayload(remainingCap, currency),
    spendCapConfigured,
    totalPrepayBalance: moneyPayload(totalPrepayBalance, currency),
    warning: financeWarning || ""
  };
}

async function mergeFreshFinance(cachedDashboard, options) {
  const dashboard = cloneJson(cachedDashboard);
  const financeData = await getMetaFinanceData(options);
  dashboard.account = financeData.account;
  dashboard.updatedAt = new Date().toISOString();
  dashboard.source = {
    ...dashboard.source,
    detail: "Performance em cache curto; financeiro atualizado diretamente pela Meta."
  };
  return dashboard;
}
...
function mapCampaign(campaign = {}, insights = {}, currency) {
  ...
  return {
    ...
    resultBreakdown: resultMetric.breakdown,
    eventBreakdown: resultMetric.eventBreakdown,
    resultSource: resultMetric.source,
    resultQuality: resultMetric.quality,
    resultHint: resultMetric.hint,
...
const actionGroups = [
  ...
  },
  {
    ...
    aliases: [
      ...
      "onsite_conversion.contact_grouped",
  ...
  {
    key: "view_content",
    label: "Visualizacoes de conteudo",
    aliases: [
      "view_content",
      "omni_view_content",
      "onsite_conversion.view_content_grouped",
...
const contactActionKeys = new Set(["lead", "contact", "message"]);
const siteIntentActionKeys = new Set(["checkout", "add_to_cart"]);

function getResultMetric(actions, objective = "") {
  const groups = summarizeActionGroups(actions);
  const objectiveName = String(objective).toUpperCase();
  const purchaseGroup = groups.find((group) => group.key === "purchase");
  const contactGroups = groups.filter((group) => contactActionKeys.has(group.key));
  const siteIntentGroups = groups.filter((group) => siteIntentActionKeys.has(group.key));
  const linkClickGroup = groups.find((group) => group.key === "link_click");
  const contactTotal = sumActionGroups(contactGroups);
  const siteIntentTotal = sumActionGroups(siteIntentGroups);
  const isSalesObjective =
    objectiveName.includes("SALES") || objectiveName.includes("CONVERSION");

  if (isSalesObjective && purchaseGroup?.count > 0) {
    return resultPayload({
      count: purchaseGroup.count,
      label: "Compras",
      breakdown: [purchaseGroup],
      eventBreakdown: groups,
      source: "purchase",
      quality: "conversion",
      hint: "Compra retornada pela Meta."
    });
  }
  ...
  if (contactTotal > 0) {
    return resultPayload({
      ...
      breakdown: contactGroups,
      eventBreakdown: groups,
      source: "contact_events",
      quality: "contact",
      hint:
        "Inclui Lead de formulario, Contato do site e Conversa iniciada quando a Meta retorna esses eventos."
    });
  }

  if (siteIntentTotal > 0) {
    return resultPayload({
      count: siteIntentTotal,
      label: "Intencoes no site",
      breakdown: siteIntentGroups,
      eventBreakdown: groups,
      source: "site_intent_events",
      quality: "intent",
      hint:
        "Nao houve Lead/Contato retornado; o painel esta mostrando eventos fortes de intencao no site."
    });
  }

  if (linkClickGroup?.count > 0) {
    return resultPayload({
      count: linkClickGroup.count,
      label: "Cliques no link",
      breakdown: [linkClickGroup],
      eventBreakdown: groups,
      source: "link_click",
      quality: "traffic",
      hint: "Sem evento de contato retornado pela Meta neste recorte."
    });
  }
  ...
  return resultPayload({
    ...
    source: "unavailable",
    quality: "empty",
    hint: "A Meta nao retornou eventos de resultado para este recorte."
  });
}

function summarizeActionGroups(actions) {
  const actionMap = new Map();

  for (const action of actions || []) {
    const type = normalizeActionType(action.action_type);
    const value = numberOrZero(action.value);
    if (!type || value <= 0) continue;
    actionMap.set(type, Math.max(actionMap.get(type) || 0, value));
  }
  ...
  return actionGroups
    .map((group) => ({
      key: group.key,
      label: group.label,
      count: Math.round(maxActionValue(actionMap, group.aliases))
    }))
    .filter((group) => group.count > 0);
}

function maxActionValue(actionMap, aliases) {
  return aliases.reduce(
    (highest, alias) => Math.max(highest, actionMap.get(normalizeActionType(alias)) || 0),
    0
  );
}

function sumActionGroups(groups) {
  return groups.reduce((sum, group) => sum + group.count, 0);
}

function resultPayload({ count, label, breakdown, eventBreakdown, source, quality, hint }) {
  return {
    count,
    label,
    breakdown: breakdown.map((item) => ({ ...item })),
    eventBreakdown: (eventBreakdown || breakdown).map((item) => ({ ...item })),
    source,
    quality,
    hint
  };
...
function currencyAmountToNumber(value, currency) {
  if (value === undefined || value === null || value === "") return null;

  if (typeof value === "object") {
    const hundredths = numberOrNull(value.amount_in_hundredths);
    if (hundredths !== null) return hundredths / 100;

    const amount = numberOrNull(value.amount);
    if (amount !== null) return amount;

    const offsettedAmount = numberOrNull(value.offsetted_amount);
    if (offsettedAmount !== null) return offsettedAmount;

    return null;
  }

  return metaMinorToNumber(value, currency);
}
...
function getDemoFinanceData() {
  ...
  return {
    source: {
      mode: "demo",
      label: "Demonstracao",
      detail: "Financeiro ficticio para validar a experiencia."
    },
    updatedAt: new Date().toISOString(),
    account: {
      id: "act_demo",
...
function demoFinanceAccount(currency) {
  const finance = buildFinanceSnapshot({
    amountSpent: 9750,
    apiBalance: 1250,
    currency,
    isPrepay: true,
    prepayAccountBalance: 1250,
    spendCap: 11000,
    totalPrepayBalance: 1250
  });
...
function makeDemoCampaign(data) {
  ...
  const resultBreakdown = demoResultBreakdown(data.resultLabel, results);
  ...
  return {
    ...
    resultBreakdown,
    eventBreakdown: resultBreakdown,
    resultSource: "contact_events",
    resultQuality: "contact",
    resultHint: "Dados demonstrativos com a mesma leitura de eventos do painel real.",
...
function makeDemoAd(campaign, index, currency) {
  ...
  const resultBreakdown = demoResultBreakdown(campaign.resultLabel, results);
  ...
  return {
    ...
    resultBreakdown,
    eventBreakdown: resultBreakdown,
    resultSource: campaign.resultSource,
    resultQuality: campaign.resultQuality,
    resultHint: campaign.resultHint,
...
function demoResultBreakdown(label, count) {
  if (!count) return [];
  const key = label === "Conversas" ? "message" : "lead";
  const breakdownLabel = label === "Conversas" ? "Conversas" : "Leads de formulario";
  return [{ key, label: breakdownLabel, count }];
}
