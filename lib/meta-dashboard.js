...
async function fetchAdsWithInsights(accountId, currency, range, options, campaigns) {
  ...
  try {
    ...
    const [adRows, insightRows] = await Promise.all([
      fetchAdRows(accountId, range, options),
      fetchAdInsights(accountId, range, options)
    ]);
    ...
    if (range.key === "today") {
      adRows
        .filter((ad) => ad.effective_status === "ACTIVE" && campaignById.has(ad.campaign_id))
        .forEach((ad) => ids.add(ad.id));
    }

    const ads = [...ids]
      .map((id) => mapAd(adRowsById.get(id), insightsById.get(id), campaignById, currency))
      .filter((ad) => range.key === "today" || ad.spendRaw > 0)
      .sort((a, b) => b.spendRaw - a.spendRaw)
      .slice(0, 24);

    return { ads: await attachAdPreviews(ads, options), warning: "" };
  } catch (error) {
    return {
      ads: [],
      warning:
...
async function attachAdPreviews(ads, options) {
  return Promise.all(
    ads.map(async (ad, index) => {
      if (ad.previewUrl || index >= 12) return ad;
      const previewUrl = await fetchAdPreviewUrl(ad, options);
      return previewUrl ? { ...ad, previewUrl } : ad;
    })
  );
}
